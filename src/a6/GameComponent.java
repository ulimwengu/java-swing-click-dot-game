...
public class GameComponent extends JPanel implements MouseListener {
    ...
    private void timeout() {
        ...
        /*
         * Compared the max game target with the target count whic is 0;
         * if the maxtarget is greater then make the game reappear/repawn
         * repaint else kill the game.
         */
        if (maxTargets > targetCount) {
            target.respawn(getWidth() - target.radius, getHeight() - target.radius);
            targetCount+= 1;
            repaint();
        } else {
            stopGame();
    ...
    public void paintComponent(Graphics g) {
        ...
        /*
         * When game is Inactive, painted black
         * If game is active, draw target at current location on top of default JPanel background
         */
        if (isActive) {
            target.paintDot(g);
        } else {
    ...
    public void mousePressed(MouseEvent m) {
        ...
        /*
         * Gotten The X and Y coordinates of the mouse press can be found using `MouseEvent.getX` and
        `.getY`.

         * Used setScore method tto set the score
         */
        if (isActive && target.checkHit(m.getX(), m.getY())) {
            setScore(score + 1);
        }
        repaint();
    ...
    private static class Target {
        ...
        void paintDot(Graphics g) {
            ...
            /*
             * if isHit color is red with 2 as radius
             * centered paint dot             
             */
            if (isHit) {
        ...
        boolean checkHit(int cx, int cy) {
            ...
            /*
             *Use Math library i.e Math.sqrt for square
             *Replaced the throw exception method.
             */
